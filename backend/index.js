    ...
    try {
        if (req.query.especie != undefined) {
            res.send(await realizarQuery(`SELECT * FROM Animales WHERE especie = '${req.query.especie}'`))
             ...
             res.send(await realizarQuery(`SELECT * FROM Animales`))
        }
    } catch (e) {
        res.send(e.message);
    }  
...
app.post('/animales',async function(req,res) {
        console.log(req.body) //Los pedidos post reciben los datos del req.body
        try {
            await realizarQuery(`
            INSERT INTO Animales (Nombre,Especie,Edad) VALUES
                ("${req.body.nombre}","${req.body.especie}",${req.body.edad});
            `)
            //El back te convierte solito a JSON siempre y cuando mande un objeto
            res.send({respuesta: "Animal agregado"})
        } catch (error) {
            res.send({respuesta: "Tuviste un error: ", error:error.message})
        }
})
